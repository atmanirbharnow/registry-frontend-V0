...
interface PublicImpactSummaryProps {
    isSchool?: boolean;
    formValues: Record<string, any>;
    userProfile?: any;
}

export default function PublicImpactSummary({
    isSchool = false,
    formValues,
    userProfile,
    ...
    if (isSchool) {
        try {
            const res = calculateSchoolImpact({
                baselineEnergyGrid: Number(formValues.baselineEnergyGrid || formValues.baseline_energy_grid) || 0,
                baselineEnergyDiesel: Number(formValues.baselineEnergyDiesel || formValues.baseline_energy_diesel) || 0,
                baselineEnergySolar: Number(formValues.baselineEnergySolar || formValues.baseline_energy_solar) || 0,
                baselineWaterMunicipal: Number(formValues.baselineWaterMunicipal || formValues.baseline_water_municipal) || 0,
                baselineWaterRain: Number(formValues.baselineWaterRain || formValues.baseline_water_rain) || 0,
                baselineWaterWaste: Number(formValues.baselineWaterWaste || formValues.baseline_water_waste) || 0,
                baselineWasteOrganic: Number(formValues.baselineWasteOrganic || formValues.baseline_waste_organic) || 0,
                baselineWasteInorganic: Number(formValues.baselineWasteInorganic || formValues.baseline_waste_inorganic) || 0,
                baselineWasteHazardous: Number(formValues.baselineWasteHazardous || formValues.baseline_waste_hazardous) || 0,
        ...
        const res = calculateImpactPhase2({
            actionType: formValues.actionType || formValues.action_type || "",
            quantity: Number(formValues.quantity || formValues.actionQuantity) || 0,
...
function UnifiedDetailsGrid({ isSchool, values, userProfile }: { isSchool: boolean; values: Record<string, any>; userProfile?: any }) {
    // Collect categories
    const categories = [];
    const hasEnergy = Number(values.baselineEnergyGrid || values.baseline_energy_grid || 0) + Number(values.baselineEnergyDiesel || values.baseline_energy_diesel || 0) + Number(values.baselineEnergySolar || values.baseline_energy_solar || 0) > 0;
    const hasWater = Number(values.baselineWaterMunicipal || values.baseline_water_municipal || 0) + Number(values.baselineWaterRain || values.baseline_water_rain || 0) + Number(values.baselineWaterWaste || values.baseline_water_waste || 0) > 0;
    const hasWaste = Number(values.baselineWasteOrganic || values.baseline_waste_organic || 0) + Number(values.baselineWasteInorganic || values.baseline_waste_inorganic || 0) + Number(values.baselineWasteHazardous || values.baseline_waste_hazardous || 0) > 0;
    ...
    const beneficiaryValue = String(values.students_count || values.beneficiariesCount || values.beneficiaries_count || "—");
